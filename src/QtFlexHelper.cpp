...
{
public:
    bool isMarginsDirty() const
    {
        return reinterpret_cast<QWidgetPrivate*>(d_ptr.data())->data.fstrut_dirty;
    }

public:
    void setMargins(int l, int t, int r, int b)
    {
        QWidgetPrivate* d = reinterpret_cast<QWidgetPrivate*>(d_ptr.data());

        QTLWExtra* topextra = d->topData();

        if (d->data.fstrut_dirty)
        {
            topextra->frameStrut.setCoords(l, t, r, b); d->data.fstrut_dirty = false;
        }
    }
};
...
void FlexHelperImplWin::updateFrame(HWND hwnd)
{
    ...
    auto proxy = static_cast<WidgetProxy*>(QWidget::find(hwnd));
    ...
    if (_wndW != w || _wndH != h)
    {
        if (_dwmEnabled)
        {
            ...
            {
                if (proxy->isMarginsDirty())
                {
                    proxy->setMargins(1, 1, 1, 1);
                }
            }
        }
        else
...
bool FlexHelper::winEvent(MSG* msg, long* result)
{
    auto d = static_cast<FlexHelperImplWin*>(impl.data());
    ...
    case WM_GETMINMAXINFO:
    {
        *result = DefWindowProc(hwnd, message, wParam, lParam);
        ...
        lpmmi->ptMinTrackSize.x = max(lpmmi->ptMinTrackSize.x, (LONG)xMin);
        lpmmi->ptMinTrackSize.y = max(lpmmi->ptMinTrackSize.y, (LONG)yMin);
    ...
    if (d->_dwmEnabled)
    {
        ...
        case WM_NCHITTEST:
        {
            ...
            QWidget* window = QWidget::find(hwnd);
...
void FlexHelper::on_button_clicked()
{
    ...
    if (!accepted)
    {
        ...
        case Flex::Minimize:
            ...
            {
#ifdef Q_OS_WIN
                ::SendMessage(window->internalWinId(), WM_SYSCOMMAND, SC_MINIMIZE, 0);
#endif
            }
            break;
        case Flex::Maximize:
            ...
            {
#ifdef Q_OS_WIN
                SendMessage(window->internalWinId(), WM_SYSCOMMAND, SC_MAXIMIZE, 0);
#endif
            }
            break;
        case Flex::Restore:
            ...
            {
#ifdef Q_OS_WIN
                SendMessage(window->internalWinId(), WM_SYSCOMMAND, SC_RESTORE, 0);
#endif
            }
            break;
        case Flex::Close:
            ...
            {
#ifdef Q_OS_WIN
                SendMessage(window->internalWinId(), WM_SYSCOMMAND, SC_CLOSE, 0);
#endif
            }
            break;
        }
